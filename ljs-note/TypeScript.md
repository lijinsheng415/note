...
**TypeScript 是 JavaScript 的一个超集**，主要提供了**类型系统**和**对 ES6+ 的支持**，它由 Microsoft 开发，代码[开源于 GitHub (opens new window)](https://github.com/Microsoft/TypeScript "开源于 GitHub (opens new window)")上
...
1. 在命令行上，运行 TypeScript 编译器：`tsc helloworld.ts`
   
   输出结果为一个 `helloworld.js` 文件，它包含了和输入文件中相同的 JavsScript 代码。

2. 在命令行上，通过 Node.js 运行这段代码：`node helloworld.js`
...
使用 `: 数据类型` ,可以给函数的参数 、变量限制参数的类型,如果限制为`string`时,就不可以传递其他类型的参数,传递其他类型会报错,不传递参数也会报错,要注意的是尽管有错误，`.js` 文件还是会被创建。 就算你的代码里有错误，你仍然可以使用TypeScript。但在这种情况下，TypeScript 会警告你代码可能不会按预期执行。
...
创建一个对象里面存放函数接收参数限制的数据类型,只有当数据和数据类型一样才可以.

## 类

```typescript
class User {
    fullName: string;  //设置类里面的类型注解
    firstName: string; //设置类里面的类型注解
    lastName: string;  //设置类里面的类型注解
...
类的声明上会注明所有的成员变量，这样比较一目了然。

# 使用webpack打包TS

### 初始化项目

- 初始化webpack: `npm init -y`&#x20;
...
或
npm i -D typescript@4.0.5 webpack@4.41.5 webpack-cli@3.3.10 
webpack-dev-server@3.10.2 html-webpack-plugin@4.5.0 clean-webpack-plugin@4.0.0
ts-loader@8.0.11 cross-env@7.0.2
```
...
这个例子展示了最基本的继承：类从基类中继承了属性和方法。 这里，`Dog` 是一个 派生类，它派生自 `Animal` 基类，通过 `extends` 关键字。 派生类通常被称作\_子类\_，基类通常被称作\_超类\_。
...
- 默认为 public  :    我们可以自由的访问程序里定义的成员。 如果你对其它语言中的类比较了解，就会注意到我们在之前的代码里并没有使用 `public` 来做修饰；例如，C# 要求必须明确地使用 `public` 指定成员是可见的。 在 TypeScript 里，成员都默认为 `public`。
- private当成员被标记成 `private` 时，它就不能在声明它的类的外部访问。
...
`readonly`可以对类中的属性成员进行修饰属性设置为只读的,修饰后该属性成员就不能在外部随意更改, 只读属性必须在声明时或构造函数里被初始化,类的普通函数和类外部无法进行操作.
...
如果在构造函数的形参之前使用`readonly` 也可以表示在类中创建了一个只读的参数
...
在形参中也可以使用`private`,`private`,`protected` 修饰符
...
静态成员:在类中通过 `static`修饰的属性或者方法,就是静态属性及静态方法,也称之为:静态成员,静态成员在使用时通过 `类名.xxx`调用
...
抽象类中包含抽象方法(抽象方法一般没有任何的具体实现),也可以包含实例方法,抽象类时不能被实例化,为了让子类进行实例化及实现内部的抽象方法.抽象类需要使用`abstract`关键字.
...
给每个参数添加类型之后再为函数本身添加返回值类型。TypeScript 能够根据返回语句自动推断出返回值类型。
...
&#x20;      使用`any`类型时接收的数据使用方法时不会出现提示,使用泛型后调用方法时可以出现智能提示.
...
&#x20;      在函数定义时使用`<T>`(尖括号中可以使任意字母)创建泛型.相当于创建了一个变量,然后在需要用变量的地方书写, 最后在调用这个函数的时候为其传入所需要的数据类型`<数据类型>`.
